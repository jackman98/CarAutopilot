...
private:
  ...
  int m_lat;
  int m_long;
  std::set<void(*)(std::shared_ptr<Car>)> m_BSMcallbacks;
  ...
  void handle_BSM_receive(std::shared_ptr<BSMMessage>);
  void handle_EVA_receive(std::shared_ptr<EVAMessage>);
...
public:
  void addBSMcallback(void (*Callback)(std::shared_ptr<Car>)){ m_BSMcallbacks.insert(Callback);}
